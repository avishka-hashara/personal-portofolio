...
export default function ChatRoom() {
  ...
  const sendMessage = async (e) => {
    ...
    // 1. Validation: Don't send empty messages
    if (!message.trim()) return;

    // 2. Clear the input immediately (makes it feel faster)
    const textToSend = message; 
    setMessage(""); // <--- THIS IS THE LINE THAT CLEARS THE BOX
    ...
    try {
      ...
      setMessage(textToSend); // Put text back if it failed
    }
  };
      ...
      {user && (
        <div className="flex justify-between items-center mb-4 border-b border-gray-700 pb-3">
          <div className="flex items-center gap-3">
            <img src={user.photoURL} alt="avatar" className="w-10 h-10 rounded-full object-cover border border-zinc-600" referrerPolicy="no-referrer" />
